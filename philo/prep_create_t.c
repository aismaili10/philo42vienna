...
/*   Updated: 2024/03/31 18:28:34 by aismaili         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

static int	init_prep(pthread_mutex_t *death, pthread_mutex_t *write)
{
...
static int init_philos(pthread_mutex_t *fork_mutex, t_input *input, t_philo *philo)
{
	...
	if (init_prep(death_lock, write_lock) == -1)
	...
	while (++i < input->num_philo)
	{
		...
		init_philos_mutex(&philo[i], death_lock, write_lock);
